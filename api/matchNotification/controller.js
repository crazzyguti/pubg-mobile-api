...
if (process.env.NODE_ENV === 'development') {
  ...
  // const sendMatchNotification = (req, res) => {
  ...
  //         // to: params.email,
  //         to: ['vbvclash@gmail.com', 'brotherhoodwithoutbannersss@gmail.com', 'vbvdhoreeeeeeeeeeeee@gmail.com'],
  ...
  const sendMatchNotification = (req, res) => {
    const schema = Joi.object().keys({
      ...
      matchTitle: Joi.string().required(),
    ...
    return Joi.validate(req.body, schema, function (err, params) {
      if (err) {
        ...
        const send = require('gmail-send')({
          ...
          subject: 'test subject',
          // text:    'gmail-send example 1',         // Plain text
          html: `<div class="container" style="position: relative; text-align: center; black: #b3e837; height:350px;width:500px">
          <div>
              <div style="position: absolute;top: 8px;left: 16px;black:white;"><p>Room Id : ${params.roomId}</p>
              <p>Room Password : ${params.roomPassword}</p>
              <p>Start Time : ${params.matchTime}</p></div>
              <div style="position: absolute;top: 70%;color:#ed580e;left: 50%;transform: translate(-50%, -50%);">GET READY</div>
              <img src="https://www.godisageek.com/wp-content/uploads/pubg-790x444.jpg" alt="Snow" style="width:500px;">
              </div>
          </div>
`            // HTML
        });
        send({ // Overriding default parameters
          subject: 'PubG Room Credentials',         // Override value set as default
        }, function (err, res) {
